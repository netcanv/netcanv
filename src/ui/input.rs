...
/// A convenience action for checking if either `a` or `b` equal to `result`.
#[derive(Debug, Clone, Copy)]
pub struct OrAction<A, R>
where
   A: Action<Result = R> + Clone,
   R: PartialEq,
{
   pub a: A,
   pub b: A,
...
impl<A, R> Action for OrAction<A, R>
where
   A: Action<Result = R> + Clone,
   R: PartialEq,
{
   ...
   fn check(&self, input: &Input) -> Self::Result {
      if input.action(self.a.clone()).eq(&self.result) {
         true
      } else if input.action(self.b.clone()).eq(&self.result) {
         true
      } else {
         false
      }
...
/// Constructs an `OrAction` in a shorthand way.
pub fn or<A, R>(a: A, b: A, result: R) -> OrAction<A, R>
where
   A: Action<Result = R> + Clone,
   R: PartialEq,
{
